...
def cnn_train():
    ...
    Output = [layers.Dense(65, activation='softmax', name='c%d' % (i + 1))(x) for i in range(size)]  # 7个输出分别对应车牌7个字符，每个输出都为65个类别类概率
    ...
    # 模型训练
    print("开始训练cnn")
    model.fit(X_train, y_train, epochs=5*size)  # 总loss为7个loss的和
    model.save('cnn.h5')
    print('cnn.h5保存成功!!!')
...
def cnn_predict(cnn, Lic_img):
    ...
    for lic in Lic_img:
        ...
        lic_pred = np.array(lic_pred).reshape(size, 65)  # 列表转为ndarray，形状为(7,65)
